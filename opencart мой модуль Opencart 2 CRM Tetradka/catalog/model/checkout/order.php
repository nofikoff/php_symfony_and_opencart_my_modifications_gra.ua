...
class ModelCheckoutOrder extends Model
{
    ...
    // это метод из КОНТРОЛЕРА из старого магазина, я перенес его сюда в МОДЕЛЬ нового
    // .... переписываю потом подменю на addOrder ($data) метода нового магазина в этой моделе
    public function addOrder($data)
    {

        //by Novikov 2018 sms вынести потом в конфиг магазина
        define('SMS_SENDER', 'gra.ua');
        define('SMS_GATE', 'http://turbosms.in.ua/api/wsdl.html');
        define('SMS_USER', 'graua');
        define('SMS_PASS', '5124315');
  ...
  (
      ...
      [store_id] =&gt; 0
      [store_name] =&gt; Modern
      [store_url] =&gt; http://test.gra.ua/
      [customer_id] =&gt; 0
      [customer_group_id] =&gt;
      [firstname] =&gt; test
      [lastname] =&gt; test
      [email] =&gt; test2@novikov.ua
      [telephone] =&gt; 876876876
      [fax] =&gt;
      [custom_field] =&gt; Array
          (
      ...
      [payment_company] =&gt;
      [payment_address_1] =&gt; deedwedwedwedwe
      [payment_address_2] =&gt;
      [payment_city] =&gt; Kiev
      [payment_postcode] =&gt; Kiev
      [payment_zone] =&gt; Kyyiv
      [payment_zone_id] =&gt; 3491
      ...
      [shipping_company] =&gt;
      [shipping_address_1] =&gt; deedwedwedwedwe
      [shipping_address_2] =&gt;
      [shipping_city] =&gt; Kiev
      [shipping_postcode] =&gt; Kiev
      [shipping_zone] =&gt; Kyyiv
      [shipping_zone_id] =&gt; 3491
      ...
      [products] =&gt; Array
          (
              [0] =&gt; Array
                  (
                      [product_id] =&gt; 9777
                      [name] =&gt; MONOPOLY Here &amp; Now WORLD Edition | Монополия Всемирное издание на английском языке
                      [model] =&gt; 693011
                      [option] =&gt; Array
                          (
                          )
              ...
              [1] =&gt; Array
                  (
                      [product_id] =&gt; 251237
                      [name] =&gt; Scrabble копия Китай - Настольная игра Тренажер для ума (на русском)
                      [model] =&gt; 895795
                      [option] =&gt; Array
                          (
                          )
      ...
      [accept_language] =&gt; ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7,uk;q=0.6
        ...
        // Федяини не пользуется механизмо Филиалов - везде Киев
        $filial_id = 'Киев';

        if ($filial_id) {
            // add buyer
            $name_buyer = $data['firstname'] . ' ' . $data['lastname'];
            $buyer_id = $this->addBuyer($name_buyer, $data['telephone']);
          ...
          (
             ...
             */
            //$total = isset($data['totals'][2]['value']) ? $data['totals'][2]['value'] : $data['totals'][0]['value'];
            $total = $data['total'];
      ...
      [shipping_code] =&gt; free.free
                ...
                'address' => $data['shipping_city'] . ($data['shipping_address_1'] ? ', ' . $data['shipping_address_1'] : ''),
                'comment' => $data['comment'] ? '<b>Покупатель:</b> ' . trim(htmlspecialchars(@strip_tags($data['comment']))) . '<br/>' : '',
                'discount' => 1,
                'is_direct' => 1,
                'amount_uah' => $total,
                //'amount_usd' => $total * $currency['usd']['value'],
                //'amount_eur' => $total * $currency['eur']['value'],
                // от балды ибо нафиг не надо
                'amount_usd' => 0,
                'amount_eur' => 0,
                ...
                'created_by' => 'site',
                'updated_by' => 'site',
                'byer_history' => $name_buyer . ' : ' . $data['ip'],
            );

            // add order
            $order_id = $this->addOrder2Tetradka($order_data);
...
//            if ($_SERVER["HTTP_CF_CONNECTING_IP"] == '46.219.78.155' OR 1) {
            if (true) {
                $txt = "Vash zakaz N$order_id na summu $total hrn uspeshno oformlen!";
                // Данные авторизации
                $auth = [
                    'login' => SMS_USER,
                    'password' => SMS_PASS
...
//                    exit;
            ...
            // Филиалы не используются и склады филиальные тоже не используются - по дефолту всем пишем один склад
            $warehouse_id = 'Склад Киквидзе';
            ...
            if (isset($data['products'])) {
                foreach ($data['products'] as $product) {
                    ...
                    $this->addOrderEntry($order_entry_data, $warehouse_id);

                    // 2018 это рудимент возможно не пригодится см ниже комменты
                    $ga_products[] = array(
                        'order_id' => $order_id,
                        'product_id' => $product['product_id'],
            ...
            // это на старом сайте использовалось толи для быстрого заказа толи еще для какой херни через сесссию
...
//            );
    ...
    // by Novikov 2018 ПИШЕМ ЗАКАЗ В ТАБЛИЦЫ ТЕРТРАДКИ
    public function addOrder2Tetradka($order)
    {
    ...
    // by Novikov 2018 Add order entry
    ...
    // by Novikov 2018 Add order comment
    ...
    /***************************** ДАЛЕЕ СТАНДАРТНЫЕ МЕТОДЫ опенкарда *******************************/
    /***************************** ДАЛЕЕ СТАНДАРТНЫЕ МЕТОДЫ опенкарда *******************************/
    /***************************** ДАЛЕЕ СТАНДАРТНЫЕ МЕТОДЫ опенкарда *******************************/
    /***************************** ДАЛЕЕ СТАНДАРТНЫЕ МЕТОДЫ опенкарда *******************************/


    public function addOrder__($data)
    {
