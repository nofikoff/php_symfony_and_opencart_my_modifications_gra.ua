...
/* тупо все продукты насыпом, но мне нравится вариант Категория ее продукты, Категория ее продукты ...
...
// выводим категорию и рядом ее продукты
...
';         
		...
		$products = $this->model_catalog_product->getProductsByCategoryId($result['category_id']);
		 ...
		 foreach ($products as $product) {
			$output .= '<url>';
// это вариант с полным адресом в URL на категорию	
...
// это вариант с адресом БЕЗ категории на товап
			$output .= '<loc>' . str_replace('&', '&amp;', $this->model_tool_seo_url->rewrite(HTTP_SERVER . 'index.php?route=product/product&product_id=' . $product['product_id'])) . '</loc>';
