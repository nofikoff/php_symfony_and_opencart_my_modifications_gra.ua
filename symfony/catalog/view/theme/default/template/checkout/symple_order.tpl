...
<div class="symple_order order_page ">
    ...
    <?php } ?>
    <form action="<?php echo str_replace('&', '&amp;', $action); ?>" method="post" enctype="multipart/form-data"
          id="cart">
        ...
        <div class="cart_products">
            <div class="cart_product_item">
                ...
                <div class="<?php echo $class; ?> row">
                    <a onclick="sendCommerce('<?php echo $product['product_id']; ?>');setTimeout(function(){ $('#remove').val('<?php echo $product['key']; ?>'); $('#cart').submit()},1000);"
                       class="delete read_more"></a>
                    <div class="price"
                         id="product_total_<?php echo $product['key']; ?>"><?php echo $product['total']; ?></div>
                    <div class="qtt">
                        <a class="qtt_change button_minus"
                           onclick="$('#qtt<?php echo $key ?>').val(<?php echo ($product['quantity']-1) ?>); $('#cart').submit()">-</a>
                        <input type="text" id="qtt<?php echo $key ?>" class="quantity-lost-element"
                               name="quantity[<?php echo $product['key']; ?>]"
                               value="<?php echo $product['quantity']; ?>" size="2"/>
                        <a class="qtt_change button_plus"
                           onclick="$('#qtt<?php echo $key ?>').val(<?php echo ($product['quantity']+1) ?>); $('#cart').submit()">+</a>
                    </div>
                    <a class="image" href="<?php echo str_replace('&', '&amp;', $product['href']); ?>"><img
                                src="<?php echo $product['thumb']; ?>" alt="<?php echo $product['name']; ?>"/></a>
                    <a class="product_name"
                       href="<?php echo str_replace('&', '&amp;', $product['href']); ?>"><?php echo $product['name']; ?></a>
                </div>
                <?php } ?>
            </div>
    ...
    font-size: 1.6em;
    float: right;
    margin-bottom: 20px;"><?php echo $total['title']; ?> &nbsp;<span
                                style="color: #be0b07;"> <?php echo $total['text']; ?></span></z>
                    <?php } ?>
                </div>
            </div>
        </div>
    ...
    <div class="order_contact">
        <form id="client_form" class="form" action="<?php echo $action ?>" method="post"
              alt="catalog/view/theme/default/template/checkout/symple_order.tpl">
            ...
            <div class="order_info order-info-top">
                ...
                <?php } ?>
                <input type="text" name="name" required="required" value="<?php echo $name; ?>" id="nameinput"/>
            </div>
            <div class="order_info order-info-top">
...
?>
                ...
                <!--<input type="text" name="phone" value="<?=$phone?>"  placeholder="Только цифры" required="required" id="phonenumber"/>-->
                <input type="text" name="phone" value="<?=$phone?>" placeholder="+38(067) 123-4567" required="required"
                       id="phonenumber"/>
            </div>
            <div class="item__wrap">
                <div class="order_info item1 itm order-info-bottom">
                    ...
                    <select id="easy-nova-poshta-type" name="easy_nova_poshta_type">
                        ...
                        <option value="office"
                        <?= ($error_easy_nova_poshta_office)?'selected':'' ?>>- отделение Новой Почты</option>
                        <option value="self"
                        <?= ($error_easy_nova_poshta_address || $error_easy_nova_poshta_buildingappartment)?'selected':'' ?>
                        >- самовывоз из магазина</option>
                        <option value="address"
                        <?= ($error_easy_nova_poshta_address || $error_easy_nova_poshta_buildingappartment)?'selected':'' ?>
                        >- курьером по Киеву</option>
                        <?php /*<option value="address" <?= ($error_easy_nova_poshta_address || $error_easy_nova_poshta_buildingappartment)?'selected':'' ?>
                        >- адресная Новой Почтой</option><?php */ ?>
                    </select><br><br>
                    ...
                    <div id="easy-nova-poshta-region-block"
                         style="display: <?= ($error_easy_nova_poshta_region)?'block':'none' ?>;">
                    ...
                    <div id="easy-nova-poshta-city-block"
                         style="display: <?= ($error_easy_nova_poshta_city)?'block':'none' ?>;">
                    ...
                    <div id="easy-nova-poshta-address-block"
                         style="display: <?= ($error_easy_nova_poshta_address || $error_easy_nova_poshta_buildingappartment)?'block':'none' ?>;">
                        ...
                        <?php } ?>
                        <input id="easy-nova-poshta-street" type="text" name="easy_nova_poshta_street" value=""
                               placeholder="Введите улицу" required="required"><br><br>
                        ...
                        <?php } ?>
                        <input id="easy-nova-poshta-buildingappartmen" type="text"
                               name="easy_nova_poshta_buildingappartment" value="" placeholder="Введите адрес"
                               required="required"><br><br>
                    </div>

                    <div id="easy-nova-poshta-office-block"
                         style="display: <?= ($error_easy_nova_poshta_office)?'block':'none' ?>;">
...
-->
            <div class="order_info">
                <h4>Виды оплаты:</h4>
                <div>
                    <select id="payment-type" name="payment-type" style="display: none;">
                        <option value=""> ---</option>
                        <option value="prepayment-to-the-card">- предоплата на карту</option>
                        <option value="payment-upon-receipt">- оплата при получении</option>
                    </select>
                </div>
                <br>
            ...
            <input type="hidden" name="city_id" id="city_id"
                   value="<?php if ($city_id){echo $city_id;} else {echo 908;} ?>"/>
        </form>


        <div class="buttons">
            <a class="button_red" onclick="check_number();"><span><?php echo $text_confirm; ?></span></a>
        </div>
    </div>
...
<script type="text/javascript">
    function check_number() {
        ...
        if (name.length >= 5) {
            if (el.value.indexOf('+38') < 0) {
                alert('Заполните номер телефона!');
            } else {
                if ($("#easy-nova-poshta-type").val() === null) {
                    alert('Выберите способ доставки!');
                } else {
                    let error = false;

                    if ($("#easy-nova-poshta-type").val() == 'office' && ($('#easy-nova-poshta-city').val() == null || $('#easy-nova-poshta-office').val() == null)) {
                        alert('Заполните пожалуйста адрес доставки');
                        error = true;
                    }

                    if ($("#easy-nova-poshta-type").val() == 'address' && ($('#easy-nova-poshta-street').val() == '' || $('#easy-nova-poshta-buildingappartmen').val() == '')) {
                        alert('Заполните пожалуйста адрес доставки');
                        error = true;
                    }
...
<noscript>
    <div style="display:inline;">
        <img height="1" width="1" style="border-style:none;" alt=""
             src="//googleads.g.doubleclick.net/pagead/viewthroughconversion/999045519/?value=0&amp;label=UZB-COG2kAQQj_Ow3AM&amp;guid=ON&amp;script=0"/>
    </div>
</noscript>
    ...
    $(document).ready(function () {
        ...
        $("input#easy-nova-poshta-street").autocomplete({
            source: function (request, response) {
                $.getJSON("/index.php?route=api/easy_novaposhta/adresses", {
                    city_ref: $('input#easy-nova-poshta-city-hef').val(),
                    term: $("input#easy-nova-poshta-street").val()
                }, response);
            },
            minLength: 2,
...
<script type="text/javascript" src="/catalog/view/javascript/maskinput.js"></script>
<script type="text/javascript">
